...
class LoginTest
{
	...
	static JDatePanelImpl datepanel=new JDatePanelImpl(model, null);
	static JDatePickerImpl datepicker=new JDatePickerImpl(datepanel, null);
...
{
	//JTextField tf=new JTextField(datepicker);
	...
	LoginFrame()
	{
		...
		//pf.setEchoChar('*');
	//	pf.setFont(new Font("Algerian",Font.ITALIC,16));
		...
		bar.setMaximum(1000);
		bar.setString("...Loading To MainMenu...");
		bar.setFont(new Font("Times New Roman",Font.ITALIC,16));
			   ...
			   if (current == activity.getTarget())
			   {
				 ...
				 try
				 {
					...
					stm=new StartTeacherMessage();
				  	...
				  	if(flag)
				  	{
				  		Message m=new Message("images//Msg.jpg");
				  		synchronized(m)
				  		{
				  			m.start();
						}
		...
		Font ft=new Font("comic sans ms",Font.ITALIC,13);
		tf.setFont(ft);
		tf.setForeground(new Color(100,220,230));
		pf.setFont(ft);
		pf.setForeground(new Color(220,110,0));
