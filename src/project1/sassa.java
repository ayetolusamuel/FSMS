...
/*
  class Staff_Modify extends JFrame implements ActionListener{
 JLabel lblsId,lblfName,lblPosition,lblsofOrigin,lblpNumber,lblLGA,lblDOE,lblCert,lblAddress,lblGender,lblEmail,lblgName,lblgpNumber,lblgAddress;
	 JPanel main;
	JTextField txtsCode,txtfName,txtEmail,txtpNumber,txtLGA,txtCert,txtgName,txtgpNumber,txtPosition;
	 JComboBox cmbsofOrigin,cmbdDay,cmbdMonth,cmbdYear;
...
class StaffsInfo extends JFrame implements ActionListener
{
	...
	private JComboBox cmbdDay,cmbdMonth,cmbdYear;
	...
	private JTextField sname,txtgpNumber,txtgName;
	private JTextField mname,txtpNumber,txtCert,txtLGA;
	String[] state={"select","Abia" ,"Adamawa","Akwa-Ibom","Bauchi","Bayelsa","Benue","Borno","Cross-River","Delta","Ekiti","Ebonyi","Edo","Enugu","Gombe","Imo","Jigawa","Kaduna","Kastina","Kano","Kebbi","Kogi","Kwara","Lagos","Nassarawa","Niger","Ogun","Ondo","Osun","Oyo","Plateau","Rivers","Sokoto","Taraba","Yobe","Zamfara","FCT"};
PreparedStatement ps=null;
CheckboxGroup cbmf;
Checkbox cbm,cbf;
	...
	StaffsInfo()
	{
		...
		lblsId=new JLabel("<html><b><i>Staff ID Number:</i></b></html>");
		main.add(lblsId).setBounds(10,60,500,20);
		scode.setToolTipText("Staff Identification Number");
		scode.setFont(new Font("Times New Roman",Font.ITALIC,18));
		scode.setBorder(BorderFactory.createBevelBorder(1,new Color(192,192,255),new Color(192,192,255)));
		main.add(scode).setBounds(120,62,200,20);
		
		lblfName=new JLabel("<html><b><i>Staff Full name :</i></b></html>");
		main.add(lblfName).setBounds(10,90,500,20);
		sname=new JTextField(20);
		main.add(sname).setBounds(120,92,550,20);
		sname.addKeyListener(onlyText);
		sname.setFont(new Font("Times New Roman",Font.ITALIC,18));
		sname.setToolTipText("Enter Staff full Name");
		...
		mname.setFont(new Font("Times New Roman",Font.ITALIC,18));
		mname.addKeyListener(onlyText);
		mname.setToolTipText("Enter Staff Position");
		mname.setBorder(BorderFactory.createBevelBorder(1,new Color(192,192,255),new Color(192,192,255)));
		main.add(mname).setBounds(120,242,200,20);
	...
	//studentmaster_item.addActionListener(this);
        ...
        marks_item.setIcon(new ImageIcon("images//delete.png"));
...
//		short_term_courses_item.addActionListener(this);
     ...
     //   registration_menu.addSeparator();
      //  registration_menu.add(short_term_courses_item);
       ...
       // query_menu.add(query_builderItem);
		...
		add(new JLabel("First  Name  : "),cons,0,3,1,1);
		add(new JLabel(" Middle name  : "),cons,0,4,1,1);
	...
	class StudentInfoHandler implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			if(source==btn8)
			{
				StaffContact pass=new StaffContact();
				pass.setVisible(true);
				pass.setSize(1200, 520);
				pass.setLocation(12, 10);
			...
			if(source== stud_contact)
			{
				StaffContact rt=new StaffContact();
				rt.setSize(1200, 520);
				rt.setLocation(12, 10);
				rt.setVisible(true);
				setVisible(false);
			}
			if(source==search_stud)
			{
				...
				SearchDialog dl=new SearchDialog();
				dl.setVisible(true);
			}
...
public void insertUpdate(){
 ...
 if(txtDate.getText().equals("")||txtTime.getText().equals("")||txtfName.getText().equals("")||txtstaffID.getText().equals("")||txtemail.getText().equals("")||txtAmount.getText().equals("")||txtBonus.getText().equals("")||txtCert.getText().equals("")||txtgName.getText().equals("")||txtgName.getText().equals("")||txtgPhone.getText().equals("")||txtOthers.getText().equals("")||txtpNumber.getText().equals("")||txtPosition.getText().equals("")){
	...
	JOptionPane.showMessageDialog(null,"<html><font size=4 color=red><i><b>Fill all editable Fields</i></b></font></html>  ","Warning",JOptionPane.ERROR_MESSAGE,error);
		...
		if(txtpNumber.getText().length()==0){
